...
void trial(unsigned scenario) {
    ...
    std::jthread responder{[&](const std::stop_token& stop) {
        ...
        while (!stop.stop_requested()) {
            ...
            if (f.data[0] == std::byte{0x40}) {
                const auto width = index == 0x1017 || index == 0x6040 || index == 0x2000 || index == 0x200f
    ...
    const auto result = session.prepare();
    CHECK(result.ok() == (scenario == 0 || scenario == 7 || scenario == 28));
