...
struct SerialConfig {
    ...
    bool mark_errors{false}; // PARMRK: ff ff is data ff; ff 00 x is an error.
};
...
class SerialPort final {
   ...
   */
    [[nodiscard]] static Result<SerialPort> open(std::string path, SerialConfig config) noexcept;
   ...
   */
    [[nodiscard]] Result<std::size_t> read_some(std::span<std::byte> destination, std::chrono::milliseconds timeout,
   ...
   */
    ...
    /**
   * Read back the effective raw 8-bit configuration; fail on unsupported format.
   * @return Owned settings or ioctl/format error; not an electrical measurement.
   * Thread safety: Single-owner; does not change the descriptor.
   */
    [[nodiscard]] Result<SerialConfig> configuration() const noexcept;

    /**
   * Flush unread kernel input after a known discontinuity.
   * @return Flush status. Thread safety: Single-owner; invalidates queued bytes.
   */
    [[nodiscard]] Status discard_input() noexcept;

    /**
   * Query currently queued bytes; this is a racing snapshot, not an age estimate.
   * @return Queue size or ioctl failure. Thread safety: Single-owner.
   */
    [[nodiscard]] Result<std::size_t> pending_bytes() const noexcept;
