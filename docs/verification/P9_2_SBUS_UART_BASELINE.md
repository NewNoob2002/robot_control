...
日期：2026-09-16。状态：**CLOSED：UART 与只读观察范围验收通过；软件、CI、实机功能及现场接线资料已收口**。
...
后续30秒采集已完成通道动作对应确认。操作员补充确认：反相后的 SBUS 接 USB 转串口模块 RX，双方共地；模块当前设为 5 V、可切换 3.3 V，接收机由 5 V 供电。
这些是现场陈述，未独立测量信号幅度或核实模块 RX 耐压；供电电压不能证明信号电平。

源码提交 `c42e2a5071c6b793c41d3ffa81ef60cbda13693c` 的远端 CI 已通过：
GitHub Actions run `35051777345`，completed/success。此结果不代表后续文档提交的 CI 状态。
结合下方软件、交叉构建、静态采集、第二轮通道／失联恢复／SIGTERM证据及接线确认，P9.2 关闭。
第一轮失败记录保持 FAILED。独立波形测量和物理 USB 拔插未执行，不新增为关闭门槛；正式通道标定留给 P9.3。
本次仅更新文档，历史证据保持原样，未重新访问硬件。
...
- `robot-control-sbus-observer`：默认 100000 8E2、1 秒；显式 device，最长 60 秒，
  初版最多 4096 条 read 的限制在30秒实测中暴露并修复，现按 1 MiB 内核原始字节限额。
  输出配置、内核原始字节、接收时间、
...
| 通道动作对应、发射机失联／恢复、目标 SIGTERM | 第二轮30秒验证通过；第一轮28.7秒记录上限失败保留，见后续记录 |
| 物理电气独立测量／USB拔插／完整标定 | 未执行，不外推为通过 |
...
下表保留初版软件／静态采集对应的源码与产物身份；预算修复后的身份见后续记录，
不能把初版哈希当作当前源码哈希。
...
操作员再次授权并两次确认准备后，第一轮约28.7秒触发4096条记录限制而失败，
第二轮使用1 MiB字节预算修复产物完成30秒：4290帧，原CH1/CH3/CH6/CH7角色
得到操作员动作确认；flags依次0→4→12→0；SIGTERM退出143，确认退出约16 ms。
第二轮启动前5.5 ms内有3次候选拒绝后重同步，不能将启动原始数据全部算作完整帧。
两轮各自的原始记录、失败原因、修复回归、构建身份和现场确认见
[30秒后续验证](evidence/p9_2_sbus_manual_20260916/RESULT.md)。
...
5. 任何配置不支持、日志不完整、超时或通道不符均保持未验收；通过后追加实机记录与
   独立提交/push。未执行的电气／拔插／完整标定不冒充通过，不扩大本轮授权范围。
