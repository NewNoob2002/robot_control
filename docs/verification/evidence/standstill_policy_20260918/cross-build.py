"""Reproduce the standstill-policy qualification artifact cross build with the locked image and real target sysroot."""
import hashlib
import json
import os
...
snapshot=root/'out/standstill20-20260918-v2-cross-source'
attestation=root/'out/standstill20-20260918-v2-cross-source.json'
...
configure=['cmake','-S','.','-B','out/build/cross/standstill20-20260918-v2-runtime','-G','Ninja',
...
subprocess.run(base+['cmake','--build','out/build/cross/standstill20-20260918-v2-runtime','--parallel','2'],check=True)
subprocess.run(['scripts/sysroot/validate_sysroot.sh',str(sysroot),str(lock)],check=True)
metadata={'source':json.loads(attestation.read_text()),'image_id':image,'configure':configure,
          ...
          'artifacts':{}}
for name in ('tools/zlac_qualification/robot-control-zlac-qualification',):
 p=root/'out/build/cross/standstill20-20260918-v2-runtime'/name
 metadata['artifacts'][name]=hashlib.sha256(p.read_bytes()).hexdigest()
(root/'out/standstill20-20260918-v2-cross-metadata.json').write_text(json.dumps(metadata,indent=2)+chr(10))
