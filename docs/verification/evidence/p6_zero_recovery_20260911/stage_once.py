...
root = out.parents[3]
manifest = json.loads((out / "manifest_once.json").read_text())
base = manifest["base"]
assert base == "/tmp/robot-control-qualifications/zero-recovery-7dcb656a56d2"
...
(out/"target_vcan_once.log").write_text(result.stdout)
(out/"target_vcan_once.stderr").write_text(result.stderr)
