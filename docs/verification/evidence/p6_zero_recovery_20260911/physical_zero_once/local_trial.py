...
assert authorization['authorized'] is True and authorization['stimulus'] == 'zero'
...
go = False
started = False
frames = downloads = uploads = nmts = nonzero = terminal = recovery = 0
...
def validate_frame(frame):
    ...
    global frames, downloads, uploads, nmts, nonzero, terminal, recovery
    frames += 1
    ...
    if ident == 0x181:
        assert len(payload) == 8 and payload[4:] == bytes(4), 'nonzero or malformed zero-trial feedback'
    ...
    allowed = {(0x2B, 0x1017, 0, 0), (0x2B, 0x1017, 0, 500),
               ...
               (0x23, 0x60FF, 1, 0), (0x23, 0x60FF, 2, 0)}
...
try:
    ...
    while True:
        ...
        if label == 'jcan_out':
            ...
            if connected and receive_enabled and remote is None:
                remote = subprocess.Popen([*SSH, 'python3', '-u', BASE + '/remote_physical_zero_once.py'], stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    ...
    assert started and nonzero == 0 and terminal == 2 and recovery == 1
    ...
    if remote is not None:
        transfer = subprocess.run(['scp', '-q', '-o', 'BatchMode=yes', '-o', 'ConnectTimeout=5', '-r', 'robot-dev:' + BASE + '/physical_zero_once', str(OUT / 'target')], capture_output=True, timeout=20)
    ...
    (OUT / 'coordinator_result.json').write_text(json.dumps({'go_sent': go, 'executor_started': started, 'error': error, 'frames': frames, 'downloads': downloads, 'uploads': uploads, 'nmt_frames': nmts, 'nonzero_requests': nonzero, 'cleanup_shutdown_requests': terminal, 'disable_voltage_recovery_requests': recovery}, indent=2))
