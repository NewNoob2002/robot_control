    ...
    with (OUT / "rk3588_can.log").open("wb") as raw, (OUT / "capture.stderr").open("wb") as capture_error:
        ...
        with (OUT / "executor.log").open("wb") as log:
            ...
            while not final_power_off_seen:
                ...
                time.sleep(0.01)
            power_cut = fresh_marker("operator_power_cut.json", ready["pid"], 120)
            fresh_marker("operator_power_restore.json", ready["pid"], 90)
            fresh_marker("operator_no_restart.json", ready["pid"], 60)
            fresh_marker("operator_final_power_off.json", ready["pid"], 15)
            try:
        ...
        post = snapshot()
        assert child.returncode == 0, "power loss did not complete zero-first recovery"
