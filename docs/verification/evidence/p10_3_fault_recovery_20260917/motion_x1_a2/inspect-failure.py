...
def main():
    ...
    assert not (BASE/'recovery-target/candump.stderr').read_text()
    for _,ident,p in records:
        ...
        if ident == 0x181:
            ...
            for offset in (0,2):
                ...
                assert word & 0x6f in (0,0x07,0x21,0x23,0x27,0x40,0x60) and (offset == 0 or word & 0x8000 == 0)
        elif ident == 0x201:
    ...
    trace = module.analyze((BASE/'recovery-target/application.log').read_text())
    ...
    assert cycles and all(d[8] > 0 and d[7] == d[8] and d[5] == 0 for d in cycles)
    ...
    result = {'status':'INCOMPLETE_F3_LATE_X1_AND_FAILED_POST_STOP_NEGATIVE_FEEDBACK','matching_capture_frames':len(records),
              ...
              'input_interpretation':'Single-throttle profile: raw Source candidates remain equal; qualification projection sends only right positive. Automatic cutoff occurs before X1. No negative transmitted target.',
              ...
              'counter_limit':'Counter evidence is in separate metrics-and-counters.json'}
