...
PROMPTS = (
    ...
    ('MOTION_READY', '仅右轮：只向前推油门，转向保持中立；右轮开始转动就立即关闭手持发射机（提前熟悉关机动作），接收机持续供电。关机前不要回中，以免变成主动停车；关机后摇杆回中。保持发射机关闭直到驱动断电，不再 CH6；异常时立即 X1/断电。'),
...
def main():
    ...
    print('F4 A2：右轮运动 SBUS 发射机失联停车 — 右轮正向 ≤5rpm / 不足3秒，左轮始终静止；反馈静止容差±1.5rpm；总就绪窗口60秒。\n')
    ...
    print('摇杆中立、CH6 松开、X1 已解除复位，遥控器和接收机均已开启且连接正常；接收机全程供电，能够在右轮开始转动后立即关闭手持发射机，驱动已上电就绪，能看清本窗口。')
    print('准备好且上述全部属实后输入 START 回车；随后还须确认本次操作是关闭遥控器，其他输入取消。')
    answer = input('就绪确认 > ').strip()
    if answer != 'START' or input('请输入 关闭遥控器 确认本次刺激（不是按X1） > ').strip() != '关闭遥控器':
        print('已取消；未开始试验。')
        return
    confirmation = dict(statement='F4右轮运动SBUS失联已上电就绪', raw_user_statement=answer,
                        confirmation_source='Local interactive terminal after displayed F4 A2 checklist',
    ...
    action = input('请记录实际操作：是否在右轮转动期间关闭手持发射机、关机前是否回中、接收机是否全程供电、是否使用了 X1 > ').strip()
