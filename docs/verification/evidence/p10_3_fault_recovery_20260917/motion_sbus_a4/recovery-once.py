"""One prepared right-wheel moving SBUS loss ControlLoop qualification on the explicitly identified RK3588."""
import hashlib
import json
import signal
import re
...
SHA = '82318b05b0bdeb24b3f94137c1f375929320ee63ca9a1c887829e1365b441048'
BINARY = BASE / 'robot-control-hil'
ARGS = ['--interface', 'can0', '--device', '/dev/serial/by-id/usb-1a86_USB_Single_Serial_586D017868-if00', '--duration-ms', '60000', '--right-throttle', '--zero-feedback-tenths-rpm', '15', '--motion-window-ms', '8000']
...
def shutdown_prompt_due(contents, now, moving_since):
    """Schedule one visible shutdown cue after one second of observed right motion; never send control."""
    if moving_since is None and any(int(value) > 15 for value in re.findall(
            r'^event=control .* right_tenths_rpm=(-?\d+) ', contents, re.MULTILINE)):
        moving_since = now
    stopped = 'event=stop_cause' in contents or 'phase=control ok=0' in contents
    return moving_since, not stopped and moving_since is not None and now-moving_since >= 1.0
...
def main():
    ...
    try:
        with (out / 'candump.log').open('wb') as raw, (out / 'candump.stderr').open('wb') as err:
            ...
            with (out / 'application.log').open('wb') as log:
                ...
                while application.poll() is None:
                    ...
                    if not result.get('operator_prompted') and 'event=control_start' in contents:
                        ...
                        print('CONTROL_READY: keep sticks neutral; press CH6 once; RIGHT motion SBUS loss window60s; wait for MOTION_READY', flush=True)
                    ...
                    for phase, marker, prompt in (
                        ('motion_ready', 'event=motion_ready', 'MOTION_READY: right positive only; forward throttle ONLY, steering neutral; hold and wait for POWER_OFF_READY; maximum8s, keep receiver powered'),
                        ('stop_verified', 'event=motion_stop verified=1', 'STOP_VERIFIED: neutral; keep transmitter OFF and receiver powered; wait for cleanup then power OFF'),
                    ...
                    moving_since, due = shutdown_prompt_due(contents, time.monotonic(), moving_since)
                    if due and result.get('motion_ready') and not result.get('power_off_prompted') and not result.get('stopping_prompted') and last_remaining != 0:
                        result['power_off_prompted'] = True
                        print('POWER_OFF_READY: positive right feedback observed for1s; long-press transmitter power OFF now; keep receiver powered; hold throttle until OFF then neutral', flush=True)
                    time.sleep(0.02)
            ...
            result['acceptance'] = 'PENDING independent motion/SBUS-loss/feedback/restoration audit'
