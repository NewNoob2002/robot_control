...
def main():
    ...
    print('F5 A1：零目标 UART 静默与恢复验证 — 全程双轮应静止；目标严格为零，速度反馈容差 ±2 rpm；总窗口 60 秒。\n')
    ...
    print('摇杆中立、CH6 松开、X1 已解除复位，手持遥控器和接收机均已开启且连接正常；本次仅断开/接回已确认可单独拔插的 SBUS 信号端口；USB、接收机供电及共地全程保持，发射机保持开启；信号端口可安全触及且不会碰到其他引脚，驱动已上电就绪，能看清本窗口。')
    ...
    confirmation = dict(statement='F5零目标UART静默恢复已上电就绪', raw_user_statement=answer,
                        confirmation_source='Local interactive terminal after displayed F5 A1 checklist',
