...
def check(log):
    ...
    for line in log.splitlines():
        ...
        if row['kind']=='0':
            assert data[2]==0, 'Reader discontinuity is not silence'
    ...
    assert first_restored[0] > release and first_restored[1][1]==before[-1][1][1]
    assert not any(d[3] for t,d in batches if last_frame < t < first_restored[0]), 'Noise bytes inside valid-frame gap; not strict raw silence'
