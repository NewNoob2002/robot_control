...
def check(trace, fault):
    ...
    assert zero - first < 2950000000, 'Automatic cutoff is not SBUS fault acceptance'
    frames = trace['frames']
...
def main():
    ...
    assert fault
    result = check(trace, int(fault[1]))
    result['feedback'] = analyzer['analyze_motion_feedback'](trace, 15, True)
