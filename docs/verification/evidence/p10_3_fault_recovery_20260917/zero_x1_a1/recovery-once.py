...
ARGS = ['--interface', 'can0', '--device', '/dev/serial/by-id/usb-1a86_USB_Single_Serial_586D017868-if00', '--duration-ms', '60000', '--zero-x1-recovery']
            ...
            with (out / 'application.log').open('wb') as log:
                ...
                while application.poll() is None:
                    ...
                    for phase, prompt in (
                        ...
                        if not result.get(phase) and ('event=recovery phase='+phase+' ') in (out/'application.log').read_text():
                            result[phase] = True
