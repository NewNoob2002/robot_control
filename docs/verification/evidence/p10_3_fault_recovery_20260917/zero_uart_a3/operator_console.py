...
PROMPTS = (
    ...
    ('INPUT_RESTORED', '健康输入已恢复、仍未授权：现在油门向前推足并保持，按 CH6 一次后松开；等待回中提示。双轮仍应静止。'),
...
def main():
    ...
    print('F5 A3：零目标 UART 静默与恢复验证 — 全程双轮应静止；目标严格为零，速度反馈容差 ±2 rpm；总窗口 120 秒。\n')
    ...
    confirmation = dict(statement='F5零目标UART静默恢复已上电就绪', raw_user_statement=answer,
                        confirmation_source='Local interactive terminal after displayed F5 A3 checklist',
