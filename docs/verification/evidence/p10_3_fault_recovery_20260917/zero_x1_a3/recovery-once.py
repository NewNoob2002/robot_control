"""One prepared zero-target X1 recovery ControlLoop qualification on the explicitly identified RK3588."""
import hashlib
import json
import signal
import subprocess
...
SHA = '0d379ed8861c9c2fb06bc08502ea52cb8f6bfeaac4bdddc25515a328a094770e'
BINARY = BASE / 'robot-control-hil'
ARGS = ['--interface', 'can0', '--device', '/dev/serial/by-id/usb-1a86_USB_Single_Serial_586D017868-if00', '--duration-ms', '60000', '--zero-x1-recovery', '--zero-feedback-tenths-rpm', '10']
        ...
        with (out / 'candump.log').open('wb') as raw, (out / 'candump.stderr').open('wb') as err:
            ...
            with (out / 'application.log').open('wb') as log:
                ...
                while application.poll() is None:
                    ...
                    for phase, prompt in (
                        ('fault_ready', 'FAULT_READY: press and lock X1; wait for RELEASE_FAULT'),
                        ('release_fault', 'RELEASE_FAULT: reset X1, hold forward throttle, press/release CH6 once; wait for NEUTRAL_READY'),
