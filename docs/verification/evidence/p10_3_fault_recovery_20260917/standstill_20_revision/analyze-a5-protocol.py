...
BASE = OUT.parent / 'motion_sbus_a5'
...
def main():
    ...
    for stamp, ident, payload in records:
        ...
        if ident == 0:
            ...
            assert abs(other) <= 20 and -20 <= selected <= 75, 'feedback outside selected positive wheel envelope'
    ...
    result={'status':'PASS_revised20_protocol_and_restoration_only','matching_frames':len(records),
    ...
    (OUT/'a5-protocol-analysis.json').write_text(json.dumps(result,indent=2)+chr(10))
