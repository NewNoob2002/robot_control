...
def fixture(partial):
    ...
    text+='event=recovery phase=release_fault at_ns=2200000000\n'
    ...
    text+=row(0,2300000000,[4,restored,0,25,1])
    text+=row(1,2300000000,[4,restored,0,1000,993,200,200,0])
    return text
...
bad=[log+row(0,1600000000,[5,2,0,1,0]),
     ...
     log.replace('kind=1 ns=2300000000','kind=1 ns=2500000000')]
for value in bad:
    try:
        check(value)
