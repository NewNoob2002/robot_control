...
PROMPTS = (
    ...
    ('FAULT_READY', '现在只断开 SBUS 信号端口；USB、供电、共地保持连接，发射机保持开启；不要触碰轮子或其他端子，等待恢复提示。'),
    ('RELEASE_FAULT', '摇杆中立、CH6 松开，将 SBUS 信号接回原端口，USB/供电/共地不动；连接恢复后油门向前推足并保持，再按 CH6 一次后松开；等待回中提示。全程目标仍为零。'),
...
def main():
    ...
    print('F5 A2：零目标 UART 静默与恢复验证 — 全程双轮应静止；目标严格为零，速度反馈容差 ±2 rpm；总窗口 60 秒。\n')
    ...
    confirmation = dict(statement='F5零目标UART静默恢复已上电就绪', raw_user_statement=answer,
                        confirmation_source='Local interactive terminal after displayed F5 A2 checklist',
