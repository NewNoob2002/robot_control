"""Bounded silent capture for the P10.3 single-wheel ControlLoop trial."""
import json
import os
...
OUT = BASE / 'recovery-jcan-once'
JCAN = '/home/gtc/Desktop/workspace/JCAN/target/release/jcan'
SERIAL = '207F346D5650'


def main():
    """Receive independently for at most 120 seconds; send no CAN and never retry the trial."""
    ...
    deadline = time.monotonic() + 60  # Authentication only; the actual receive window is 120s.
                ...
                for key, _ in selector.select(timeout=0.05):
                    ...
                    while b'\n' in buffers[label]:
                        ...
                        if event == 'connected':
                            ...
                            result['frames'] += 1
                            assert result['frames'] <= 20000, 'capture frame budget exceeded'
                        else:
                        ...
                        if connected and receiving and not (OUT / 'ready.json').exists():
                            ...
                            print('JCAN_READY silent receive; maximum120s', flush=True)
            result['passed'] = True
