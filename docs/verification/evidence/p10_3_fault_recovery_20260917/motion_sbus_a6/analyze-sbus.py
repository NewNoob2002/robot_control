...
def check(trace, fault, motion_window_ms=3000):
    ...
    frames = trace['frames']
    faults = [f for f in frames if f['flags'] & 12 and f['ns'] >= first]
    if fault in (3, 4):
        ...
        assert not any(f['flags'] & 12 and first <= f['ns'] <= zero for f in frames)
    ...
    assert any(first <= t < stimulus and int.from_bytes(p[6:8], 'little', signed=True) > 20
...
def main():
    ...
    assert 'zero_feedback_tenths_rpm=20 ' in header
    ...
    assert 'feedback_hard_bad=0 ' in header
    reviews = [analyzer['fields'](line) for line in log.splitlines() if line.startswith('event=feedback_review ')]
    result['feedback_reviews'] = reviews
    if reviews:
        assert trace['feedback_bad'] and all(row['verdict'] == 'pending' for row in reviews)
        result['feedback'] = 'PENDING_OPERATOR_REVIEW; raw excursions retained'
    else:
        result['feedback'] = analyzer['analyze_motion_feedback'](trace, 20, True, 8000)
    runpy.run_path(str(BASE/'analyze-protocol.py'))['main']()
    result.update(status='PENDING_FEEDBACK_REVIEW' if reviews else 'PASS_F4_WIRE_PENDING_OPERATOR', protocol=json.loads((BASE/'protocol-analysis.json').read_text()))
