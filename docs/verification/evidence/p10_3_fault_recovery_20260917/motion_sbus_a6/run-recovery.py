...
REMOTE = '/home/cat/.cache/robot-control/staging/p103-stop-review-cd8bd1c5-sbus-a6-20260917'
...
def main():
    ...
    assert confirmation['statement'] == 'F4右轮运动SBUS失联已上电就绪'
    assert confirmation['motion_window_ms'] == 8000
    assert confirmation['power_off_prompt_after_positive_feedback_ms'] == 1000
    assert confirmation['receiver_stays_powered'] and confirmation['transmitter_only_stimulus']
    ...
    try:
        ...
        for line in (BASE / 'recovery-execution.log').read_text().splitlines():
