...
def main():
    ...
    print('A2 残留配置恢复：不使能、不运行控制循环，双轮应始终静止。')
    print('本次只核对残留后恢复原基线；全程保持中立，CH6 不按，X1 不操作。')
    print('速度反馈容差 ±1 rpm，范围内持续至少150ms；目标严格为零。')
    ...
    if answer != 'START':
        print('已取消；未开始试验。')
        return
    confirmation = dict(statement='零目标基线恢复已上电就绪', raw_user_statement=answer,
                        confirmation_source='Local interactive terminal after displayed baseline-restoration checklist',
