"""Direct, local operator instructions for one zero-only SBUS trial."""
...
PROMPTS = (
    ...
    ('FAULT_READY', '现在只断开 SBUS 信号端口；USB、供电、共地保持连接，发射机保持开启；不要触碰轮子或其他端子，断开后至少保持5秒，等允许接回提示；无需赶时间。'),
    ('RELEASE_FAULT', '摇杆中立、CH6 松开，将 SBUS 信号接回原端口，USB/供电/共地不动；接回与恢复操作窗口45秒；接回后保持中立、CH6松开，等待 INPUT_RESTORED，不要立即操作油门或CH6。'),
    ('INPUT_WAIT_STABLE', '输入再次失稳：保持摇杆中立、CH6松开，等待连续健康1秒后的新 INPUT_RESTORED 提示；旧提示已失效，不追加操作。'),
    ('INPUT_RESTORED', '输入已连续健康且中立至少1秒、仍未授权：现在油门向前推足并保持，按 CH6 一次后松开；等待回中提示。双轮仍应静止。'),
    ('NEUTRAL_READY', '现在摇杆回中，CH6 保持松开；等待重新授权提示。'),
    ('REARM_READY', '保持摇杆中立，CH6 再按一次后松开，然后等待；不要追加 CH6 操作。'),
...
def current_prompt(log):
    ...
    return (name, message, remaining) if offset >= 0 else ('WAIT', '正在准备采集和应用；保持中立，暂不操作 CH6 或拔信号线。', remaining)
...
def main():
    ...
    print('F5 A4：零目标 UART 静默与恢复验证 — 全程双轮应静止；目标严格为零，速度反馈容差 ±2 rpm；总窗口 120 秒。\n')
    ...
    print('摇杆中立、CH6 松开、X1 已解除复位，手持遥控器和接收机均已开启且连接正常；开始前确认上次拔下的信号线已接回原端口、遥控连接正常；本次仅断开/接回已确认可单独拔插的 SBUS 信号端口；USB、接收机供电及共地全程保持，发射机保持开启；信号端口可安全触及且不会碰到其他引脚，驱动已上电就绪，能看清本窗口。')
    print('准备好且上述全部属实后输入 START 回车；其他输入取消，不开始采集或控制。')
    answer = input('就绪确认 > ').strip()
    if answer != 'START':
        print('已取消；未开始试验。')
        return
    confirmation = dict(statement='F5零目标UART静默恢复已上电就绪', raw_user_statement=answer,
                        confirmation_source='Local interactive terminal after displayed F5 A4 checklist',
                        ...
                        operator_display_visible=True, receiver_stays_powered=True, signal_only_stimulus=True,
                        usb_ground_power_unchanged=True, transmitter_stays_on=True)
    ...
    print('若双轮始终未转、无异常声音、驱动已断电，且本次只按提示拔插SBUS信号、USB/供电/共地保持连接、发射机全程开启，输入 OFF；否则描述实际情况。')
    ...
    record = dict(raw_user_statement=answer, wheels_stationary=True if answer == 'OFF' else None,
                  ...
                  signal_only_stimulus=True if answer == 'OFF' else None,
                  usb_ground_power_unchanged=True if answer == 'OFF' else None,
                  transmitter_stayed_on=True if answer == 'OFF' else None,
                  confirmation_source='Local terminal post-trial question including signal-only fixture confirmation', runner_exit=result.returncode)
    (BASE / 'operator-post-trial.json').write_text(json.dumps(record, ensure_ascii=False, indent=2) + '\n')
    action = input('请记录实际操作：是否只拔插 SBUS 信号端口；USB、供电、共地是否保持；是否按恢复提示接回；发射机是否全程开启 > ').strip()
