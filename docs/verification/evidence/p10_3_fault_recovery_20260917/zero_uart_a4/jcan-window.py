...
def main():
    """Receive independently for at most 180 seconds; send no CAN and never retry the trial."""
    ...
    deadline = time.monotonic() + 60  # Authentication only; the actual receive window is 180s.
                ...
                for key, _ in selector.select(timeout=0.05):
                    ...
                    while b'\n' in buffers[label]:
                        ...
                        if event == 'connected':
                            ...
                            result['frames'] += 1
                            assert result['frames'] <= 40000, 'capture frame budget exceeded'
                        else:
                        ...
                        if connected and receiving and not (OUT / 'ready.json').exists():
                            ...
                            print('JCAN_READY silent receive; maximum180s', flush=True)
            result['passed'] = True
