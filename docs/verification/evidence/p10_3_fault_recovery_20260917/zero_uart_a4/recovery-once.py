...
ARGS = ['--interface', 'can0', '--device', '/dev/serial/by-id/usb-1a86_USB_Single_Serial_586D017868-if00', '--duration-ms', '120000', '--zero-uart-recovery', '--zero-feedback-tenths-rpm', '20']
...
def uart_messages(contents, seen):
    """Return each new acquisition transition in order, including repeated prompt invalidation."""
    events=[line for line in contents.splitlines() if line.startswith(('event=uart_resync ', 'event=uart_reconnected '))]
    messages=['INPUT_WAIT_STABLE: input unstable; neutral sticks and release CH6; wait for new INPUT_RESTORED' if line.startswith('event=uart_resync ') else 'INPUT_RESTORED: healthy neutral input stable1s; hold forward throttle, press/release CH6 once; wait for NEUTRAL_READY' for line in events[seen:]]
    return messages, len(events)
...
def main():
    ...
    try:
        with (out / 'candump.log').open('wb') as raw, (out / 'candump.stderr').open('wb') as err:
            capture = subprocess.Popen(['stdbuf', '-oL', 'candump', '-D', '-ta', '-e', '-n', '40000',
            ...
            with (out / 'application.log').open('wb') as log:
                ...
                while application.poll() is None:
                    ...
                    assert time.monotonic() - started < 150, 'application exceeded 150s guard'
                    ...
                    if not result.get('operator_prompted') and 'event=control_start' in contents:
                        ...
                        print('CONTROL_READY: keep sticks neutral; press CH6 once; ZERO recovery window120s; wait for FAULT_READY', flush=True)
                    if control_started is not None:
                        remaining = max(0, 119 - int(time.monotonic() - control_started))
                    ...
                    for phase, prompt in (
                        ...
                        ('release_fault', 'RELEASE_FAULT: neutral sticks and CH6 released, reconnect SBUS SIGNAL to original port; USB, ground and power unchanged; 45s reconnect/challenge window; remain neutral and CH6 released; wait for INPUT_RESTORED'),
                        ('input_restored', 'INPUT_RESTORED: healthy input; hold forward throttle, press/release CH6 once; wait for NEUTRAL_READY'),
                        ...
                        if phase == 'input_restored':
                            messages, uart_seen = uart_messages(contents, uart_seen)
                            if last_remaining != 0 and not result.get('stopping_prompted'):
                                for message in messages:
                                    print(message, flush=True)
                            continue
                        if not result.get(phase) and ('event=uart_reconnected ' if phase == 'input_restored' else 'event=recovery phase='+phase+' ') in contents and last_remaining != 0 and not result.get('stopping_prompted'):
                            result[phase] = True
