...
PROMPTS = (
    ...
    ('MOTION_READY', '仅右轮：只向前推油门，转向保持中立，另一只手放在 X1 上；右轮一开始转动就立即按锁 X1，随后摇杆回中。保持 X1 锁定直到退出并关闭驱动电源；不要再次 CH6。'),
...
def main():
    ...
    print('F3 A3：右轮运动 X1 急停 — 右轮正向 ≤5rpm / 不足3秒，左轮始终静止；反馈静止容差±1.5rpm；总就绪窗口60秒。\n')
    ...
    confirmation = dict(statement='F3右轮运动X1已上电就绪', raw_user_statement=answer,
                        confirmation_source='Local interactive terminal after displayed F3 A3 checklist',
    ...
    print(f'\n采集执行器已返回 {result.returncode}（急停结果待独立审查）；保持摇杆中立，请关闭驱动电源。')
