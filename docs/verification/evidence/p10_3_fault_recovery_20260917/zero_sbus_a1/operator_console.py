...
PROMPTS = (
    ...
    ('FAULT_READY', '现在关闭发射机电源；保持关闭，等待恢复提示。X1 保持解除。'),
    ('RELEASE_FAULT', '摇杆中立、CH6 松开，重新开启发射机；连接恢复后油门向前推足并保持，再按 CH6 一次后松开；等待回中提示。'),
...
def main():
    ...
    print('F2 A1：零目标 SBUS 恢复验证 — 全程双轮应静止；目标严格为零，速度反馈容差 ±1 rpm；总窗口 60 秒。\n')
    ...
    print('摇杆中立、CH6 松开、X1 已解除复位，发射机已开启且连接正常，驱动已上电就绪，能看清本窗口。')
    ...
    confirmation = dict(statement='F2零目标SBUS恢复已上电就绪', raw_user_statement=answer,
                        confirmation_source='Local interactive terminal after displayed F2 A1 checklist',
