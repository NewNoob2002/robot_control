        ...
        with (out / 'candump.log').open('wb') as raw, (out / 'candump.stderr').open('wb') as err:
            ...
            with (out / 'application.log').open('wb') as log:
                ...
                while application.poll() is None:
                    ...
                    for phase, prompt in (
                        ('fault_ready', 'FAULT_READY: turn transmitter OFF; keep X1 released; wait for RELEASE_FAULT'),
                        ('release_fault', 'RELEASE_FAULT: neutral sticks and CH6 released, turn transmitter ON; after reconnection hold forward throttle, press/release CH6 once; wait for NEUTRAL_READY'),
