...
snapshot=root/'out/p103-standstill15-v1-cross-source'
attestation=root/'out/p103-standstill15-v1-cross-source.json'
...
configure=['cmake','-S','.','-B','out/build/cross/p103-standstill15-v1-runtime','-G','Ninja',
...
subprocess.run(base+['cmake','--build','out/build/cross/p103-standstill15-v1-runtime','--parallel','2'],check=True)
subprocess.run(['scripts/sysroot/validate_sysroot.sh',str(sysroot),str(lock)],check=True)
...
for name in ('tools/control_hil/robot-control-hil','robot_control_control_cycle_tests'):
 p=root/'out/build/cross/p103-standstill15-v1-runtime'/name
 metadata['artifacts'][name]=hashlib.sha256(p.read_bytes()).hexdigest()
(root/'out/p103-standstill15-v1-cross-metadata.json').write_text(json.dumps(metadata,indent=2)+chr(10))
