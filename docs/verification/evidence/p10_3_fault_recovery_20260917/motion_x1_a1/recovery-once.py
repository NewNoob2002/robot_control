            ...
            with (out / 'application.log').open('wb') as log:
                ...
                while application.poll() is None:
                    ...
                    for phase, marker, prompt in (
                        ('motion_ready', 'event=motion_ready', 'MOTION_READY: right positive only; forward throttle plus left steering; press/lock X1 immediately when wheel moves; neutral after locking; keep X1 locked through power OFF'),
