...
def check(tx, rx):
    ...
    assert any(first <= t < x1 and int.from_bytes(p[6:8],'little',signed=True) > 0 for t,p in status), 'no physical motion before X1'
...
def main():
    ...
    assert len(trace['stops']) == 1 and trace['stops'][0]['cause'] in (1,5)
