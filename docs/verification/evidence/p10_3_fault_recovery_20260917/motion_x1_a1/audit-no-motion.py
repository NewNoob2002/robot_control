...
def main(incomplete=False):
    ...
    if incomplete:
        ...
        recovery = {'kind':'x1','status':'NOT_EXERCISED_INPUT_GATE_REJECTED'}
    ...
    for stamp, ident, payload in records:
        ...
        if ident == 0:
            ...
            assert len(payload) == 8 and all(abs(int.from_bytes(payload[o:o+2], 'little', signed=True)) <= 10 for o in (4,6))
            ...
            if pending[0] == 0x40:
                ...
                if index == 0x606c:
                    data = payload[4:8]
                    assert all(abs(int.from_bytes(part, 'little', signed=True)) <= 10 for part in ([data[:2],data[2:]] if sub == 3 else [data]))
    ...
    log = (BASE/'recovery-target/application.log').read_text()
    summary = re.search(r'event=summary cycles=(\d+) enabled_samples=(\d+).*primary_ok=' + '0' + r' restore_ok=1', log)
    ...
    assert application['passed'] is False and application['application_exit'] == 1
    result={'status':'INCOMPLETE_SCENARIO_protocol_and_restoration_verified' if incomplete else 'INCOMPLETE_F3_ZERO_TARGET_PROTOCOL_AND_RESTORATION_VERIFIED','matching_frames':len(records),
    ...
    (BASE/('incomplete-analysis.json' if incomplete else 'no-motion-protocol-analysis.json')).write_text(json.dumps(result,indent=2)+chr(10))
    print(json.dumps(result,indent=2))


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--incomplete", action="store_true", help="Audit failed F2 protocol/restoration only; never grant recovery acceptance")
