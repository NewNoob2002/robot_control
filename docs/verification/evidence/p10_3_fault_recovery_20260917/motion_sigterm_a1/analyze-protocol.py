...
def main():
    ...
    assert '--motion-window-ms' not in auth['arguments']
    ...
    for stamp, ident, payload in records:
        ...
        if ident == 0:
            ...
            assert abs(other) <= 20 and (-75 if zero_after else -20) <= selected <= 75, 'feedback outside hard envelope'
            ...
            if pending[0] == 0x40:
                ...
                if index == 0x606c:
                    ...
                    assert all(abs(v-(1 << bits) if v & (1 << (bits-1)) else v) <= 20 for v in parts)
    ...
    assert len(tpdo2_times) >= 2 and tpdo2_times[-1]-tpdo2_times[0] >= 1.9
    assert 6 in words and 7 in words and 15 in words
    assert words.index(6) < words.index(7) < words.index(15)
    assert len(enabled_feedback) >= 2 and len(rpdo_times) >= 20
    log = (BASE/'recovery-target/application.log').read_text()
    summary = re.search(r'event=summary cycles=(\d+) enabled_samples=(\d+).*primary_ok=[01] restore_ok=1', log)
    ...
    assert application['application_exit'] in (0, 1)
    assert application['passed'] == (application['application_exit'] == 0)
    assert nonzero and zero_after and 0 < zero_after[0]-nonzero[0] < 3.0
    assert all(abs(speed) <= 20 for stamp,speed in velocities if stamp < nonzero[0])
    assert sum(speed > 20 for stamp,speed in velocities if nonzero[0] <= stamp <= zero_after[0]) >= 2
    stopped = [(stamp,speed) for stamp,speed in velocities if stamp >= zero_after[0]]
    last_moving = max((stamp for stamp,speed in stopped if abs(speed) > 20), default=zero_after[0])
    stable = [stamp for stamp,speed in stopped if stamp > last_moving and abs(speed) <= 20]
    assert len(stable) >= 3 and stable[-1]-stable[0] >= 0.15
    assert stable[0]-zero_after[0] <= 1.0
    assert 'event=motion_stop verified=1' in log
    result={'status':'PASS_motion_protocol_and_restoration_only_NOT_F6_acceptance','matching_frames':len(records),
