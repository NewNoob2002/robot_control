            ...
            with (out / 'application.log').open('wb') as log:
                ...
                while application.poll() is None:
                    ...
                    for phase, marker, prompt in (
                        ('motion_ready', 'event=motion_ready', 'MOTION_READY: right positive only; forward throttle ONLY, steering neutral, switch transmitter OFF immediately when wheel moves; keep receiver powered; neutral after switching OFF; keep transmitter OFF through drive power OFF'),
