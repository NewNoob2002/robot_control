...
def main(incomplete=False):
    ...
    if incomplete:
        ...
        assert phases == [('fault_ready', 'x1')], phases
        assert 'operation=control_hil_recovery_incomplete' in log
        recovery = {'kind': 'x1', 'status': 'INCOMPLETE', 'phases': phases}
    else:
        recovery = module['analyze_recovery'](log, 10)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--incomplete", action="store_true", help="Audit failed F1 protocol/restoration only; never grant recovery acceptance")
