...
PROMPTS = (
    ...
    ('FAULT_READY', '现在只关闭手持遥控器（发射机）；接收机保持供电，不拔线。等待恢复提示，X1 保持解除。'),
    ('RELEASE_FAULT', '摇杆中立、CH6 松开，重新开启手持遥控器（发射机）；接收机继续供电；连接恢复后油门向前推足并保持，再按 CH6 一次后松开；等待回中提示。'),
...
def current_prompt(log):
    ...
    return (name, message, remaining) if offset >= 0 else ('WAIT', '正在准备采集和应用；保持中立，暂不操作 CH6 或关闭发射机。', remaining)
...
def main():
    ...
    print('F2 A2：零目标 SBUS 恢复验证 — 全程双轮应静止；目标严格为零，速度反馈容差 ±1 rpm；总窗口 60 秒。\n')
    ...
    print('摇杆中立、CH6 松开、X1 已解除复位，手持遥控器和接收机均已开启且连接正常；本次仅开关手持遥控器，接收机全程供电，驱动已上电就绪，能看清本窗口。')
    print('准备好且上述全部属实后输入 START 回车；其他输入取消，不开始采集或控制。')
    answer = input('就绪确认 > ').strip()
    if answer != 'START':
        print('已取消；未开始试验。')
        return
    confirmation = dict(statement='F2零目标SBUS恢复已上电就绪', raw_user_statement=answer,
                        confirmation_source='Local interactive terminal after displayed F2 A2 checklist',
    ...
    action = input('请记录实际操作：关闭/开启了哪个设备，接收机是否全程供电 > ').strip()
