...
SHA = '616e8ad3ec59c14985106c846c8f706b396617dc16b226b3012e3509fd821cea'
BINARY = BASE / 'robot-control-hil'
ARGS = ['--interface', 'can0', '--device', '/dev/serial/by-id/usb-1a86_USB_Single_Serial_586D017868-if00', '--duration-ms', '60000', '--zero-sbus-recovery', '--zero-feedback-tenths-rpm', '10']
        ...
        with (out / 'candump.log').open('wb') as raw, (out / 'candump.stderr').open('wb') as err:
            ...
            with (out / 'application.log').open('wb') as log:
                ...
                while application.poll() is None:
                    ...
                    for phase, prompt in (
                        ('fault_ready', 'FAULT_READY: turn HANDHELD TRANSMITTER OFF; RECEIVER MUST STAY POWERED; keep X1 released; wait for RELEASE_FAULT'),
                        ('release_fault', 'RELEASE_FAULT: neutral sticks and CH6 released, turn HANDHELD TRANSMITTER ON; keep receiver powered; after reconnection hold forward throttle, press/release CH6 once; wait for NEUTRAL_READY'),
