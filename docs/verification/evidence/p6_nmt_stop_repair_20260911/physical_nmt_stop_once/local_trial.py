...
SERIAL = '207F346D5650'
BASE = '/tmp/robot-control-qualifications/nmt-repair-202bab381749'
SSH = ['ssh', '-o', 'BatchMode=yes', '-o', 'ConnectTimeout=5', 'robot-dev']
authorization = json.loads((OUT / 'authorization.json').read_text())
assert authorization['authorized'] is True and authorization['stimulus'] == 'nmt_stop'
assert authorization['elf_sha256'] == '202bab381749c0418a61d727a7de02490d4a625594bba3e98346c6f96d3f175d'
...
def validate_frame(frame):
    ...
    if ident == 0:
        assert go and payload in (bytes([1, 1]), bytes([0x80, 1]), bytes([2, 1])), frame
        assert not (nmt_stop and payload == bytes([1, 1])), "Operational after Stop"
    ...
    if index == 0x6040 and value in (6, 7, 15):
...
try:
    ...
    while True:
        ...
        if label == 'jcan_out':
            ...
            if connected and receive_enabled and remote is None:
                remote = subprocess.Popen([*SSH, 'python3', '-u', BASE + '/remote_physical_nmt_stop_once.py'], stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    ...
    assert started and nonzero == 1 and terminal == 1 and nmt_stop == 1 and cleanup_disable == 1
    ...
    if remote is not None:
        transfer = subprocess.run(['scp', '-q', '-o', 'BatchMode=yes', '-o', 'ConnectTimeout=5', '-r', 'robot-dev:' + BASE + '/physical_nmt_stop_once', str(OUT / 'target')], capture_output=True, timeout=20)
