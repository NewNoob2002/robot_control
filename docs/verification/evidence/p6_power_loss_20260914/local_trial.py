...
SERIAL = "207F346D5650"
BASE = "/tmp/robot-control-qualifications/power-loss-fc77b14ec242"
...
try:
    ...
    while True:
        ...
        if label == "jcan_out":
            ...
            if message == "CAPTURE_READY":
                ...
                pass
            elif message in ("OUTCOME ZERO_FIRST_RECOVERY", "OUTCOME INHIBITED_INTERFACE_DOWN"):
    ...
    if remote_outcome == "ZERO_FIRST_RECOVERY":
        assert first_host_after_boot == (0x601, "23ff600300000000"), first_host_after_boot
    else:
        assert first_host_after_boot is None, first_host_after_boot
    ...
    (OUT / "coordinator_result.json").write_text(json.dumps({
        ...
        "host_requests_after_error": host_after_error, "remote_outcome": remote_outcome}, indent=2) + "\n")
