"""Reproduce the P10.2 cross build with the locked image and real target sysroot."""
import hashlib
import json
import os
...
snapshot=root/'out/p102-reviewed-cross-source'
attestation=root/'out/p102-reviewed-cross-source.json'
...
configure=['cmake','-S','.','-B','out/build/cross/p102-reviewed-runtime','-G','Ninja',
...
subprocess.run(base+['cmake','--build','out/build/cross/p102-reviewed-runtime','--parallel','2'],check=True)
subprocess.run(['scripts/sysroot/validate_sysroot.sh',str(sysroot),str(lock)],check=True)
metadata={'source':json.loads(attestation.read_text()),'image_id':image,'configure':configure,
          ...
          'artifacts':{}}
for name in ('robot_control_control_loop_vcan_tests','robot_control_control_cycle_tests','robot_control_canopen_runtime_vcan_tests','robot_control_canopen_runtime_policy_tests'):
 p=root/'out/build/cross/p102-reviewed-runtime'/name
 metadata['artifacts'][name]=hashlib.sha256(p.read_bytes()).hexdigest()
(root/'out/p102-reviewed-cross-metadata.json').write_text(json.dumps(metadata,indent=2)+chr(10))
