"""Audit P10.2 application loop linkage and ABI without target execution."""
from pathlib import Path
import re
import subprocess

binary = Path('out/build/cross/p102-reviewed-runtime/robot_control_control_loop_vcan_tests')
...
for method in ('ControlLoop::step(', 'ControlCycle::prepare(', 'ControlCycle::complete(', 'RuntimeSession::submit(', 'RuntimeSession::stop(', 'Lifecycle::run_until(', 'Reader::read(', 'ControlArbiter::evaluate(', 'SafetyManager::evaluate(', 'RuntimePolicy::evaluate(', 'Source::consume('):
...
print('PASS: single runtime guard, application loop, CAN lifecycle and UART/source linked; upstream TX deny retained; qualification/commissioning gates absent')
