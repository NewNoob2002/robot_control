...
BASE = Path('/tmp/robot-control-qualifications/p66-manual-tpdo-8ca2c0250736')
ELF = BASE / 'robot-control-zlac-qualification'
EXPECTED = '8ca2c0250736e792ecb0b7067973ce2547f7e2d6fb9c38d0147a5788ade34c8a'
    ...
    with (OUT / 'rk3588_can.log').open('wb') as raw, (OUT / 'capture.stderr').open('wb') as err:
        ...
        with (OUT / 'executor.log').open('wb') as log:
            ...
            while child.poll() is None:
                if not ready_reported and 'MANUAL_ROTATION_READY' in (OUT / 'executor.log').read_text():
                    ready_reported = True
                    print('MANUAL_ROTATION_READY duration_ms=60000 no_motor_commands=1', flush=True)
