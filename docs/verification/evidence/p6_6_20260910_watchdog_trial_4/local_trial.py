...
SERIAL = '207F346D5650'
BASE = '/tmp/robot-control-qualifications/p66-watchdog-ea1fba5cb9fa'
...
assert authorization['elf_sha256'] == 'ea1fba5cb9fa3c6e5ede192820b4a4ddc6af575a404f46e19d8290497e7b2ea5'
...
try:
    ...
    while True:
        ...
        if label == 'jcan_out':
            ...
            if connected and receive_enabled and remote is None:
                remote = subprocess.Popen([*SSH, 'python3', '-u', BASE + '/remote_trial_4.py'], stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    ...
    if remote is not None:
        transfer = subprocess.run(['scp', '-q', '-o', 'BatchMode=yes', '-o', 'ConnectTimeout=5', '-r', 'robot-dev:' + BASE + '/trial_4', str(OUT / 'target')], capture_output=True, timeout=20)
