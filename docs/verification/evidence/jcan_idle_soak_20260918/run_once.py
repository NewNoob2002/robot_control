#!/usr/bin/env python3
"""Bound one authorized three-hour JCAN normal-mode idle receive session."""
import hashlib
import json
import os
...
def main():
    ...
    with (HERE / 'session.jsonl').open('wb') as raw, (HERE / 'session.stderr').open('wb') as stderr, \
        ...
        try:
            ...
            write_json('preflight.json', {
                ...
                'operator_state': {'drive_power': 'ON', 'x1_locked': False, 'continuous_can_traffic': False},
                ...
                'limitation': 'Idle receive only; no traffic-load or motor safety acceptance'})
                    ...
                    for _key, _mask in selector.select(timeout=0.2):
                        ...
                        while b'\n' in buffer:
                            ...
                            if event.get('event') == 'connected':
                                ...
                                print('RUNNING: normal receive; three-hour idle window; no data sends', flush=True)
    ...
    result = {'status': 'FAIL' if error else 'PASS_IDLE_WINDOW' if complete else 'INTERRUPTED',
              ...
              'limitation': 'No load qualification or proof of permanent repair; USB raw transfers not captured'}
