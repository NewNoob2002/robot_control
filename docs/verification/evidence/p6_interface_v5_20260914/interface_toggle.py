...
assert not (BASE/'interface_loss_manual_v5_once').exists(), 'Test already started; do not retry'
...
print('READY: tell the agent this terminal is ready. Wait for the new trial. Watch the RIGHT wheel; press Enter ONCE promptly when it moves (within 6 seconds). Motion ends by 8 seconds if not triggered.',flush=True)
...
arm=BASE/'interface_loss_manual_v5_once/armed.json'
...
assert data['elf_sha256']==EXPECTED and data['moving_tpdo_observed'] and 0<=time.time()-data['wall_time']<6
