...
SERIAL = '207F346D5650'
BASE = '/tmp/robot-control-qualifications/interface-v5-2304c1c9892d'
...
assert authorization['elf_sha256'] == '2304c1c9892d327f73c65304601adfd4ced30e81fb2d4f3bfd1e2c5966ae857c'
...
try:
    ...
    while True:
        ...
        if label == 'jcan_out':
            ...
            if connected and receive_enabled and remote is None:
                remote = subprocess.Popen([*SSH, 'python3', '-u', BASE + '/remote_interface_loss_manual_v5_once.py'], stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    ...
    if remote is not None:
        transfer = subprocess.run(['scp', '-q', '-o', 'BatchMode=yes', '-o', 'ConnectTimeout=5', '-r', 'robot-dev:' + BASE + '/interface_loss_manual_v5_once', str(OUT / 'target')], capture_output=True, timeout=20)
