...
out = Path(__file__).resolve().parent / "right"
frames = []
...
commands = [f for f in frames if f[1] == 0x201 and f[2] == bytes.fromhex('0F0000000500')]
assert len(commands) == 1
start = commands[0][0]
zero = next(f[0] for f in frames if f[0] > start and f[1] == 0x201 and f[2] == bytes.fromhex('060000000000'))
assert sum(i == 0x201 for t,i,d,n in frames) == 2
...
assert all(reads[k] == v for k,v in {(0x1400,1):0x201,(0x1400,2):255,(0x1400,5):1000,(0x1600,0):2,(0x1600,1):0x60400010,(0x1600,2):0x60600008}.items())
...
tx = sum(i in (0,0x601,0x201) for t,i,d,n in frames)
assert post["stats64"]["tx"]["packets"]-pre["stats64"]["tx"]["packets"] == tx
for direction in ("rx","tx"):
    selected = [f for f in frames if (f[1] in (0,0x601,0x201)) == (direction == "tx")]
