...
BASE = Path('/tmp/robot-control-qualifications/review-rk3588-74206353ffd3')
ELF = BASE / 'robot-control-zlac-qualification'
EXPECTED = '74206353ffd3f0c8b93547f2b88350d9edcf50862839015e348b708896deded0'
authorization = json.loads((BASE / 'authorization_trial_2.json').read_text())
...
try:
    with (OUT / 'rk3588_can.log').open('wb') as raw, (OUT / 'capture.stderr').open('wb') as err:
        ...
        with (OUT / 'executor.log').open('wb') as log:
            child = subprocess.Popen([str(ELF), '--interface', 'can0', '--manual-tpdo'], stdout=log, stderr=subprocess.STDOUT)
            started = True
            ...
            while child.poll() is None:
                ...
                for line in lines[reported_lines:]:
                    if line.startswith('MANUAL_ROTATION_READY'):
                        sample_seen = True
                    if 'error' in line.lower() or 'warning' in line.lower():
                        allowed = line == '(CO_epoll_processLast) CAN Epoll error, events=0x01, fd=7'
                        if line == '(CO_CANsend) OS error "Permission denied" in send()' and not permission_seen and not sample_seen:
                            allowed = True
                            permission_seen = True
                        assert allowed, 'new application diagnostic: ' + line
                        if line not in known_debug:
                            print('KNOWN_DEBUG_DIAGNOSTIC ' + line, flush=True)
                            known_debug.add(line)
                    if line.startswith(('MANUAL_ROTATION_READY', 'manual_sample ', 'MANUAL_ROTATION_END')):
