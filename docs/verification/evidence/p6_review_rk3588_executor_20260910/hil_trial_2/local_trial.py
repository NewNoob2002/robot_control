...
SERIAL = '207F346D5650'
BASE = '/tmp/robot-control-qualifications/review-rk3588-74206353ffd3'
SSH = ['ssh', '-o', 'BatchMode=yes', '-o', 'ConnectTimeout=5', 'robot-dev']
authorization = json.loads((OUT / 'authorization.json').read_text())
assert authorization['authorized'] is True and authorization['stimulus'] == 'manual_tpdo'
assert authorization['elf_sha256'] == '74206353ffd3f0c8b93547f2b88350d9edcf50862839015e348b708896deded0'
...
def validate_frame(frame):
    ...
    if command == 0x40:
        allowed = {(0x6060, 0), (0x6061, 0), (0x1017, 0), (0x603F, 0), (0x6041, 0), (0x60FF, 1), (0x60FF, 2)}
...
try:
    ...
    while True:
        ...
        if label == 'jcan_out':
            ...
            if connected and receive_enabled and remote is None:
                remote = subprocess.Popen([*SSH, 'python3', '-u', BASE + '/remote_trial_2.py'], stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
            ...
            if message == 'CAPTURE_READY':
                ...
                started = True
            elif message.startswith(('MANUAL_ROTATION_READY', 'manual_sample ', 'MANUAL_ROTATION_END', 'KNOWN_DEBUG_DIAGNOSTIC ')):
                pass
            elif not message.startswith('TRIAL_FINISHED rc=0'):
                raise RuntimeError(message)
        elif label == 'jcan_out_eof':
            raise RuntimeError('JCAN disconnected')
    assert started and nonzero == 0 and terminal == 0
    ...
    if remote is not None:
        transfer = subprocess.run(['scp', '-q', '-o', 'BatchMode=yes', '-o', 'ConnectTimeout=5', '-r', 'robot-dev:' + BASE + '/trial_2', str(OUT / 'target')], capture_output=True, timeout=20)
