"""Reproduce the P10.3 initial diagnostics gate cross build with the locked image and real target sysroot."""
import hashlib
import json
import os
...
snapshot=root/'out/p103-diagnostics-reviewed-cross-source'
attestation=root/'out/p103-diagnostics-reviewed-cross-source.json'
...
configure=['cmake','-S','.','-B','out/build/cross/p103-diagnostics-reviewed-runtime','-G','Ninja',
           '-DCMAKE_TOOLCHAIN_FILE=cmake/toolchains/aarch64-rk3588-ubuntu2204.cmake',
           '-DCMAKE_BUILD_TYPE=Debug','-DBUILD_TESTING=ON','-DROBOT_CONTROL_BUILD_ZLAC_QUALIFICATION=ON']
subprocess.run(base+configure,check=True)
subprocess.run(base+['cmake','--build','out/build/cross/p103-diagnostics-reviewed-runtime','--parallel','2'],check=True)
subprocess.run(['scripts/sysroot/validate_sysroot.sh',str(sysroot),str(lock)],check=True)
metadata={'source':json.loads(attestation.read_text()),'image_id':image,'configure':configure,
          ...
          'artifacts':{}}
for name in ('tools/zlac_qualification/robot-control-zlac-qualification','robot_control_control_cycle_tests'):
 p=root/'out/build/cross/p103-diagnostics-reviewed-runtime'/name
 metadata['artifacts'][name]=hashlib.sha256(p.read_bytes()).hexdigest()
(root/'out/p103-diagnostics-reviewed-cross-metadata.json').write_text(json.dumps(metadata,indent=2)+chr(10))
