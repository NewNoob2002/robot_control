"""One prepared no-motion TPDO2 qualification on the explicitly identified RK3588."""
import hashlib
import json
import signal
import subprocess
...
BINARY = BASE / 'robot-control-zlac-qualification'
ARGS = ['--interface', 'can0', '--runtime-diagnostics']
        ...
        with (out / 'candump.log').open('wb') as raw, (out / 'candump.stderr').open('wb') as err:
            ...
            with (out / 'application.log').open('wb') as log:
                ...
                while application.poll() is None:
                    ...
                    assert time.monotonic() - started < 20, 'application exceeded 20s guard'
                    time.sleep(0.02)
