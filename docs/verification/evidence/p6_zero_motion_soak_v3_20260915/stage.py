#!/usr/bin/env python3
"""Stage the authorized v3 three-hour soak runner without operating CAN."""
import hashlib
import json
import shlex
import subprocess
...
ROOT = OUT.parents[3]
BASE = "/tmp/robot-control-qualifications/zero-motion-soak-v3-bb4f44251672"
...
assert subprocess.run([*SSH, "test ! -e " + shlex.quote(BASE)], timeout=10).returncode == 0
subprocess.run([*SSH, "mkdir -p " + shlex.quote(BASE)], check=True, timeout=10)
manifest = {"status": "AUTHORIZED_ACK_SOAK_STAGED", "target": identity, "base": BASE,
...
print("PASS: authorized v3 three-hour soak runner staged; no CAN operation started")
