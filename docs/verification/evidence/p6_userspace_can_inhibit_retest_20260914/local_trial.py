...
assert authorization['qualification_sha256'] == 'fc77b14e78c60ad00f3fcda704889d0c913be60ef416e77c7ba5cd9a688b6fa4'
assert authorization['inhibitor_sha256'] == 'c2425001cbe583e3bb1e3307d4145e95cafd722d150154ff727fe519a80b1a5b'
assert authorization['user_confirmation']
...
go = False
started = False
frames = downloads = uploads = nmts = nonzero = terminal = recovery = nmt_stop = 0
startup_recovery = cleanup_disable = 0
connected = receive_enabled = False
baseline = None
interface_down_at = None
late_host_requests = 0
...
def validate_frame(frame, received_at):
    ...
    global frames, downloads, uploads, nmts, nonzero, terminal, recovery, nmt_stop, startup_recovery, cleanup_disable, late_host_requests
    frames += 1
    ...
    if interface_down_at is not None and received_at > interface_down_at + 0.25 and ident in (0, 0x201, 0x601):
        late_host_requests += 1
    ...
    if command == 0x40:
        allowed = {(0x6060, 0), (0x6061, 0), (0x1017, 0), (0x603F, 0), (0x6041, 0), (0x200F, 0), (0x2000, 0), (0x605A, 0), (0x60FF, 1), (0x60FF, 2), (0x60FF, 3)}
    ...
    allowed = {(0x2B, 0x1017, 0, 0), (0x2B, 0x1017, 0, 500),
               ...
               (0x2B, 0x6040, 0, 0), (0x2B, 0x6040, 0, 7), (0x2B, 0x6040, 0, 15),
               (0x23, 0x60FF, 3, 0), (0x2B, 0x2000, 0, 1000), (0x2B, 0x2000, 0, 0)}
    allowed |= {(0x23, 0x60FF, 3, 327680)}
    assert (command, index, sub, value) in allowed, frame
    if index == 0x60FF and value != 0:
        nonzero += 1
        assert nonzero <= 1
    if index == 0x6040 and value == 0:
        recovery += 1
        assert recovery <= 2
        if nonzero:
            cleanup_disable += 1
            assert cleanup_disable <= 1
        else:
            startup_recovery += 1
            assert startup_recovery <= 1 and nonzero == 0
    if index == 0x6040 and value in (7, 15):
        assert nonzero == 0 and nmt_stop == 0, "activation or Shutdown after motion"
...
try:
    ...
    while True:
        ...
        if label == 'jcan_out':
            ...
            if message == 'CAPTURE_READY':
                ...
                started = True
            elif message.startswith('INTERFACE_DOWN '):
                assert interface_down_at is None
                interface_down_at = received_at
            elif message in ('MANUAL_DISCONNECT_ARMED', 'OUTCOME INHIBITED_INTERFACE_DOWN'):
                pass
            elif not message.startswith('TRIAL_FINISHED rc=0'):
                raise RuntimeError(message)
        elif label == 'jcan_out_eof':
            raise RuntimeError('JCAN disconnected')
    assert started and nonzero == 1 and terminal <= 2 and nmt_stop == 0 and cleanup_disable in (0, 1)
    assert interface_down_at is not None and late_host_requests == 0
    ...
    if remote is not None:
        transfer = subprocess.run(['scp', '-q', '-o', 'BatchMode=yes', '-o', 'ConnectTimeout=5', '-r', 'robot-dev:' + BASE + '/physical_once', str(OUT / 'target')], capture_output=True, timeout=20)
    ...
    (OUT / 'coordinator_result.json').write_text(json.dumps({'go_sent': go, 'executor_started': started, 'error': error, 'frames': frames, 'downloads': downloads, 'uploads': uploads, 'nmt_frames': nmts, 'nonzero_requests': nonzero, 'cleanup_shutdown_requests': terminal, 'disable_voltage_requests': recovery, 'nmt_stop_requests': nmt_stop, 'startup_recovery_requests': startup_recovery, 'cleanup_disable_requests': cleanup_disable, 'interface_down_observed': interface_down_at is not None, 'host_requests_after_down_plus_250ms': late_host_requests}, indent=2))
