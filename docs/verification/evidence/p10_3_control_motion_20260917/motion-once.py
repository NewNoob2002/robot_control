...
SHA = '3c2d6a0fb35eac571e4a732731258e480e35925a9013ada6eca4d2a8dbd8d1f6'
BINARY = BASE / 'robot-control-hil'
ARGS = ['--interface', 'can0', '--device', '/dev/serial/by-id/usb-1a86_USB_Single_Serial_586D017868-if00', '--duration-ms', '20000', '--single-left']
...
def main():
    ...
    try:
        with (out / 'candump.log').open('wb') as raw, (out / 'candump.stderr').open('wb') as err:
            capture = subprocess.Popen(['stdbuf', '-oL', 'candump', '-D', '-ta', '-e', '-n', '10000',
            ...
            with (out / 'application.log').open('wb') as log:
                ...
                while application.poll() is None:
                    ...
                    assert time.monotonic() - started < 45, 'application exceeded 45s guard'
                    ...
                    if not result.get('operator_prompted') and 'event=control_start' in (out / 'application.log').read_text():
                        ...
                        print('CONTROL_READY: keep sticks neutral; press CH6 once; LEFT trial window20s; wait for MOTION_READY before stick input', flush=True)
