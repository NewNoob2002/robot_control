...
def main():
    """Receive independently for at most 70 seconds; send no CAN and never retry the trial."""
    ...
    deadline = time.monotonic() + 60  # Authentication only; the actual receive window is 70s.
                ...
                for key, _ in selector.select(timeout=0.05):
                    ...
                    while b'\n' in buffers[label]:
                        ...
                        if connected and receiving and not (OUT / 'ready.json').exists():
                            ...
                            print('JCAN_READY silent receive; maximum70s', flush=True)
            result['passed'] = True
