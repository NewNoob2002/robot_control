#!/usr/bin/env python3
"""Coordinate one approved Quick Stop run and two bounded passive captures."""
import json, queue, subprocess, sys, threading, time
from pathlib import Path
OUT = Path(__file__).resolve().parent
JCAN = '/home/gtc/Desktop/workspace/JCAN/target/release/jcan'
SERIAL = '207F346D5650'
BASE = '/tmp/robot-control-qualifications/p66-quick-stop-6193a8df071c'
...
def validate_frame(frame):
    ...
    if command == 0x40:
        allowed = {(i, 0) for i in (0x1017, 0x200F, 0x603F, 0x6040, 0x6041, 0x605A, 0x6060, 0x6061)}
    ...
    if index == 0x60FF:
        ...
        assert command == 0x2B and sub == 0 and value in (2, 6, 7, 15), frame
...
try:
    ...
    while True:
        ...
        if label == 'jcan_out':
            ...
            if connected and receive_enabled and remote is None:
                remote = subprocess.Popen([*SSH, 'python3', '-u', BASE + '/remote_trial_capture_fixed.py'], stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    ...
    if remote is not None:
        transfer = subprocess.run(['scp', '-q', '-o', 'BatchMode=yes', '-o', 'ConnectTimeout=5', '-r', 'robot-dev:' + BASE + '/trial_1_capture_fixed', str(OUT / 'target')], capture_output=True, timeout=20)
    ...
    (OUT / 'coordinator_result.json').write_text(json.dumps({'go_sent': go, 'executor_started': started, 'error': error, 'frames': frames, 'downloads': downloads, 'uploads': uploads, 'nmt_frames': nmts, 'nonzero_requests': nonzero, 'quick_stop_requests': terminal}, indent=2))
