#!/usr/bin/env python3
"""Run the single authorized Quick Stop trial with bounded passive capture."""
import hashlib, json, os, select, signal, subprocess, sys, time
from pathlib import Path
BASE = Path('/tmp/robot-control-qualifications/p66-quick-stop-6193a8df071c')
ELF = BASE / 'robot-control-zlac-qualification'
EXPECTED = '6193a8df071c92c766189d9871094aa1dcbfbb88e82e0c8e1acb5dfa7acd86ba'
...
for p in Path('/proc').iterdir():
    ...
    try:
        ...
        pass
OUT = BASE / 'trial_1_capture_fixed'
...
try:
    with (OUT / 'rk3588_can.log').open('wb') as raw, (OUT / 'capture.stderr').open('wb') as err:
        ...
        with (OUT / 'executor.log').open('wb') as log:
            child = subprocess.Popen([str(ELF), '--interface', 'can0', '--quick-stop-once', '2:5', '--duration-ms', '2000'], stdout=log, stderr=subprocess.STDOUT)
            started = True
