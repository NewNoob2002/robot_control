"""Independently compare both captures and verify the exact zero-only actual ControlLoop trial."""
...
def main():
    ...
    records = []
    for line in (BASE / 'zero-target/candump.log').read_text().splitlines():
        fields = line.split()
    ...
    independent = []
    for line in (BASE / 'zero-jcan-once/session.jsonl').read_text().splitlines():
    ...
    log = (BASE/'zero-target/application.log').read_text()
    ...
    application = json.loads((BASE/'zero-target/result.json').read_text())
    ...
    result={'status':'PASS_protocol_and_restoration','matching_frames':len(records),
            ...
            'scope':'Zero-target ControlLoop only; motion and fault trials remain open'}
    (BASE/'zero-analysis.json').write_text(json.dumps(result,indent=2)+chr(10))
