#!/usr/bin/env python3
"""Stage the moving X1 scripts without enabling authorization or CAN."""
import hashlib
import json
import shlex
import subprocess
...
remote_elf = subprocess.check_output([*SSH, "sha256sum " + shlex.quote(ELF)], text=True, timeout=10).split()[0]
assert remote_elf == ELF_SHA
assert subprocess.check_output([*SSH, "stat -c '%U %G %a' " + shlex.quote(ELF)], text=True, timeout=10).strip() == "root cat 750"
...
subprocess.run([*SSH, "python3 -m py_compile " + " ".join(shlex.quote(f"{BASE}/{name}") for name in NAMES if name.endswith(".py"))],
               check=True, timeout=10)
(OUT / "manifest.json").write_text(json.dumps(manifest, indent=2) + "\n")
print("PASS: moving X1 runner staged but not authorized; no CAN operation started")
