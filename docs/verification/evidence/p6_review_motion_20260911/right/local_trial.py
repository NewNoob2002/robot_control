#!/usr/bin/env python3
"""Coordinate one approved manual_tpdo run and two bounded passive captures."""
import json, queue, subprocess, sys, threading, time
from pathlib import Path
OUT = Path(__file__).resolve().parent
JCAN = '/home/gtc/Desktop/workspace/JCAN/target/release/jcan'
SERIAL = '207F346D5650'
BASE = '/tmp/robot-control-qualifications/review-online-ef9837d751f9'
SSH = ['ssh', '-o', 'BatchMode=yes', '-o', 'ConnectTimeout=5', 'robot-dev']
authorization = json.loads((OUT / 'authorization.json').read_text())
assert authorization['authorized'] is True and authorization['stimulus'] == 'right'
assert authorization['elf_sha256'] == 'ef9837d751f9038ddadfa76b42107a526b740e6d65f84f0aa8e468ccd236fff0'
...
def validate_frame(frame):
    ...
    if command == 0x40:
        allowed = {(0x6060, 0), (0x6061, 0), (0x1017, 0), (0x603F, 0), (0x6041, 0), (0x200F, 0), (0x60FF, 1), (0x60FF, 2)}
    ...
    if authorization['stimulus'] != 'zero':
        ...
        allowed |= {(0x2B, 0x200F, 0, 0), (0x2B, 0x200F, 0, 1), (0x23, 0x60FF, channel, 5)}
...
try:
    ...
    while True:
        ...
        if label == 'jcan_out':
            ...
            if connected and receive_enabled and remote is None:
                remote = subprocess.Popen([*SSH, 'python3', '-u', BASE + '/remote_motion_right.py'], stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    ...
    if remote is not None:
        transfer = subprocess.run(['scp', '-q', '-o', 'BatchMode=yes', '-o', 'ConnectTimeout=5', '-r', 'robot-dev:' + BASE + '/motion_right', str(OUT / 'target')], capture_output=True, timeout=20)
