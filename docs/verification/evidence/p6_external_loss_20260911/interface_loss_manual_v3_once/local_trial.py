...
SERIAL = '207F346D5650'
BASE = '/tmp/robot-control-qualifications/external-recovery-a8040dc1a783'
SSH = ['ssh', '-o', 'BatchMode=yes', '-o', 'ConnectTimeout=5', 'robot-dev']
authorization = json.loads((OUT / 'authorization.json').read_text())
assert authorization['authorized'] is True and authorization['stimulus'] == 'interface_loss'
assert authorization['elf_sha256'] == 'a8040dc1a7837a481644a00ce48293aed7cf09ccb9ac4b31255f7cb71372d3e7'
...
try:
    ...
    while True:
        ...
        if label == 'jcan_out':
            ...
            if connected and receive_enabled and remote is None:
                remote = subprocess.Popen([*SSH, 'python3', '-u', BASE + '/remote_interface_loss_manual_v3_once.py'], stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
            ...
            if message == 'CAPTURE_READY':
                ...
                started = True
            elif message.startswith('APP ') or message in ('INTERFACE_ARMED', 'INTERFACE_DOWN', 'INTERFACE_UP'):
                pass
            elif not message.startswith('TRIAL_FINISHED rc=0'):
                raise RuntimeError(message)
        elif label == 'jcan_out_eof':
            raise RuntimeError('JCAN disconnected')
    assert started and nonzero == 1 and terminal == 1 and nmt_stop == 0 and cleanup_disable == 1
    ...
    if remote is not None:
        transfer = subprocess.run(['scp', '-q', '-o', 'BatchMode=yes', '-o', 'ConnectTimeout=5', '-r', 'robot-dev:' + BASE + '/interface_loss_manual_v3_once', str(OUT / 'target')], capture_output=True, timeout=20)
