...
SERIAL = '207F346D5650'
BASE = '/tmp/robot-control-qualifications/external-loss-dc639ff99b70'
SSH = ['ssh', '-o', 'BatchMode=yes', '-o', 'ConnectTimeout=5', 'robot-dev']
authorization = json.loads((OUT / 'authorization.json').read_text())
assert authorization['authorized'] is True and authorization['stimulus'] == 'moving_sigterm'
assert authorization['elf_sha256'] == 'dc639ff99b7031b2137c6e23eb235ec3e0a3c635d9781dd10923072c02306a38'
...
try:
    ...
    while True:
        ...
        if label == 'jcan_out':
            ...
            if connected and receive_enabled and remote is None:
                remote = subprocess.Popen([*SSH, 'python3', '-u', BASE + '/remote_moving_sigterm_once.py'], stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
            ...
            if message == 'CAPTURE_READY':
                ...
                started = True
            elif message.startswith('APP ') or message == 'SIGTERM_SENT':
                pass
            elif not message.startswith('TRIAL_FINISHED rc=0'):
                raise RuntimeError(message)
        elif label == 'jcan_out_eof':
            raise RuntimeError('JCAN disconnected')
    assert started and nonzero == 1 and terminal == 2 and nmt_stop == 0 and cleanup_disable == 0
    ...
    if remote is not None:
        transfer = subprocess.run(['scp', '-q', '-o', 'BatchMode=yes', '-o', 'ConnectTimeout=5', '-r', 'robot-dev:' + BASE + '/moving_sigterm_once', str(OUT / 'target')], capture_output=True, timeout=20)
