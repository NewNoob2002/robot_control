...
authorization = json.loads((BASE / 'authorization_interface_loss_once.json').read_text())
...
try:
    with (OUT / 'rk3588_can.log').open('wb') as raw, (OUT / 'capture.stderr').open('wb') as err:
        cap = subprocess.Popen(['candump', '-D', '-ta', '-e', '-n', '10000', 'can0,0:0,#FFFFFFFF'], stdout=raw, stderr=err)
        time.sleep(0.25)
        ...
        with (OUT / 'executor.log').open('wb') as log:
            ...
            while child.poll() is None:
                ...
                if not signal_sent and target_seen is not None and time.monotonic() - target_seen >= 0.3:
                    ...
                    subprocess.run(['ip','link','set','dev','can0','down'],check=True,timeout=2)
                    signal_sent = True
                    link_down_at = time.monotonic()
                    stamp.update(monotonic_after_ns=time.monotonic_ns(),wall_after_ns=time.time_ns())
                    (OUT / 'interface_down_event.json').write_text(json.dumps(stamp,indent=2))
                    print('INTERFACE_DOWN',flush=True)
                if signal_sent and not link_restored and time.monotonic() - link_down_at >= 3:
                    stamp = {'monotonic_before_ns':time.monotonic_ns(),'wall_before_ns':time.time_ns()}
                    subprocess.run(['ip','link','set','dev','can0','up'],check=True,timeout=2)
                    link_restored = True
                    stamp.update(monotonic_after_ns=time.monotonic_ns(),wall_after_ns=time.time_ns())
                    (OUT / 'interface_up_event.json').write_text(json.dumps(stamp,indent=2))
    ...
    if signal_sent and not link_restored:
        if link_down_at is not None:
            time.sleep(max(0, 3 - (time.monotonic() - link_down_at)))
        restored = subprocess.run(['ip','link','set','dev','can0','up'],timeout=2)
        link_restored = restored.returncode == 0
