...
authorization = json.loads((BASE / 'authorization_interface_loss_manual_once.json').read_text())
...
for p in Path('/proc').iterdir():
    ...
    try:
        ...
        pass
OUT = BASE / 'interface_loss_manual_once'
OUT.mkdir(exist_ok=False)
gate = BASE / 'interface_manual_gate'
ready_path = gate / 'ready.json'
assert ready_path.stat().st_uid == 0, 'privileged terminal not ready'
ready = json.loads(ready_path.read_text())
assert ready['uid'] == 0 and time.time() - ready['wall_time'] < 300
