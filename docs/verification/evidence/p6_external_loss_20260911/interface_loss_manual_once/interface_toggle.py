...
assert not (BASE/'interface_loss_manual_once').exists(), 'Test already started; do not retry'
...
assert select.select([sys.stdin],[],[],300)[0], 'No operator trigger within5minutes; no interface change'
assert sys.stdin.readline() == '\n', 'Enter only; no interface change'
arm=BASE/'interface_loss_manual_once/armed.json'
