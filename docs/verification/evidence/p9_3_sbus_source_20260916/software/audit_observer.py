"""One-time P9.3 Source observer ABI and linkage verification against the target sysroot."""
from pathlib import Path
import re
import subprocess

binary = Path('out/build/cross/rk3588-debug/tools/sbus_observer/robot-control-sbus-observer')
...
for method in ('sbus::Reader::read(', 'sbus::protocol::Parser::consume(', 'uart::SerialPort::configuration(', 'sbus::Source::consume(', 'sbus::Source::stop('):
    assert method in symbols, method
assert not re.search(r'CanSocket|CANopen|\b(socket|send|sendto|sendmsg|CO_[A-Za-z_]+)(@|\s|$)', symbols)
print('PASS: aarch64, interpreter, dependencies, GLIBC/GLIBCXX/CXXABI, no RPATH')
print('PASS: reader/parser/UART/Source/stop linked; no CANopen/CanSocket/socket/send symbols')
