...
def analyze(root):
    ...
    for line in content.decode().splitlines():
        if line.startswith('event=read '):
            ...
            if last_frame is not None:
                ...
                state['button'] = None
            if state['valid']:
                assert state['coherent'] and state['enabled'] and state['authorization'] and not state['stop'] and state['health'] == 3
    ...
    gates = {}
    # The contract permits stale/framing prefixes: they must never authorize.
    # Require sustained held input AFTER recovery, plus zero/invalid throughout
    # the entire startup interval. Do not discard or relabel prefix observations.
    initial = [s for s in sources if 0 <= s['second'] < 3]
    qualified = [s for s in initial if s['health'] == 2]
    gates['startup_held_disabled'] = (bool(initial) and bool(qualified)
        and all(not s['valid'] and s['authorization'] == 0 and s['left_rpm'] == s['right_rpm'] == 0 for s in initial)
        and all(s['button'] >= 1500 and s['recovery'] >= profile['recovery_frames'] for s in qualified)
        and qualified[-1]['second'] - qualified[0]['second'] >= 1)
    ...
    return dict(capture_sha256=hashlib.sha256(content).hexdigest(), frames=len(frames), reads=len(reads),
                ...
                startup_qualified_held_seconds=[qualified[0]['second'], qualified[-1]['second']] if qualified else [],
                startup_prefix_buttons=[s['button'] for s in initial if s['health'] != 2],
                gate_revision="v2: preserve startup prefix; require zero/invalid for all startup and sustained held/disabled after recovery",
