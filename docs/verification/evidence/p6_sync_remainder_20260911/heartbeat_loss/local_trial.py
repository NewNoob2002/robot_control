...
assert authorization['authorized'] is True and authorization['stimulus'] == 'heartbeat_loss'
...
def validate_frame(frame):
    ...
    if authorization['stimulus'] != 'zero':
        channel = 1 if authorization['stimulus'] == 'heartbeat_loss' else 2
...
try:
    ...
    while True:
        ...
        if label == 'jcan_out':
            ...
            if connected and receive_enabled and remote is None:
                remote = subprocess.Popen([*SSH, 'python3', '-u', BASE + '/remote_heartbeat_loss.py'], stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    ...
    if remote is not None:
        transfer = subprocess.run(['scp', '-q', '-o', 'BatchMode=yes', '-o', 'ConnectTimeout=5', '-r', 'robot-dev:' + BASE + '/heartbeat_loss', str(OUT / 'target')], capture_output=True, timeout=20)
