...
assert authorization['authorized'] is True and authorization['stimulus'] == 'quick_stop'
...
def validate_frame(frame):
    ...
    if authorization['stimulus'] != 'zero':
        channel = 1 if authorization['stimulus'] == 'quick_stop' else 2
        allowed |= {(0x23, 0x60FF, 3, 327680)}
    allowed |= {(0x2B, 0x6040, 0, 2)}
...
try:
    ...
    while True:
        ...
        if label == 'jcan_out':
            ...
            if connected and receive_enabled and remote is None:
                remote = subprocess.Popen([*SSH, 'python3', '-u', BASE + '/remote_quick_stop.py'], stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    ...
    if remote is not None:
        transfer = subprocess.run(['scp', '-q', '-o', 'BatchMode=yes', '-o', 'ConnectTimeout=5', '-r', 'robot-dev:' + BASE + '/quick_stop', str(OUT / 'target')], capture_output=True, timeout=20)
