...
assert authorization['authorized'] is True and authorization['stimulus'] == 'disable_voltage'
...
def validate_frame(frame):
    ...
    if authorization['stimulus'] != 'zero':
        channel = 1 if authorization['stimulus'] == 'disable_voltage' else 2
        allowed |= {(0x23, 0x60FF, 3, 327680)}
    allowed |= {(0x2B, 0x6040, 0, 0)}
...
try:
    ...
    while True:
        ...
        if label == 'jcan_out':
            ...
            if connected and receive_enabled and remote is None:
                remote = subprocess.Popen([*SSH, 'python3', '-u', BASE + '/remote_disable_voltage.py'], stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    ...
    assert started and nonzero == (0 if authorization['stimulus'] == 'zero' else 1) and terminal == 1
    ...
    if remote is not None:
        transfer = subprocess.run(['scp', '-q', '-o', 'BatchMode=yes', '-o', 'ConnectTimeout=5', '-r', 'robot-dev:' + BASE + '/disable_voltage', str(OUT / 'target')], capture_output=True, timeout=20)
