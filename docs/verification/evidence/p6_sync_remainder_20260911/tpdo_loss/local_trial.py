...
SERIAL = '207F346D5650'
BASE = '/tmp/robot-control-qualifications/sync-remainder-fa1944fdba2d'
SSH = ['ssh', '-o', 'BatchMode=yes', '-o', 'ConnectTimeout=5', 'robot-dev']
authorization = json.loads((OUT / 'authorization.json').read_text())
assert authorization['authorized'] is True and authorization['stimulus'] == 'tpdo_loss'
assert authorization['elf_sha256'] == 'fa1944fdba2dd98cbe8ebbeebc34174baafe1eba0d930b33ae306fbd47091205'
...
def validate_frame(frame):
    ...
    if authorization['stimulus'] != 'zero':
        channel = 1 if authorization['stimulus'] == 'tpdo_loss' else 2
        allowed |= {(0x23, 0x60FF, 3, 327680)}
    allowed |= {(0x2B, 0x2000, 0, 1000), (0x2B, 0x2000, 0, 0)}
    allowed |= {(0x2B, 0x1800, 5, 0), (0x2B, 0x1800, 5, 100)}
...
try:
    ...
    while True:
        ...
        if label == 'jcan_out':
            ...
            if connected and receive_enabled and remote is None:
                remote = subprocess.Popen([*SSH, 'python3', '-u', BASE + '/remote_tpdo_loss.py'], stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    ...
    assert started and nonzero == (0 if authorization['stimulus'] == 'zero' else 1) and terminal == 2
    ...
    if remote is not None:
        transfer = subprocess.run(['scp', '-q', '-o', 'BatchMode=yes', '-o', 'ConnectTimeout=5', '-r', 'robot-dev:' + BASE + '/tpdo_loss', str(OUT / 'target')], capture_output=True, timeout=20)
