#!/usr/bin/env python3
"""Restore can0 once before the soak retest while drive power is off."""
import json
import os
import subprocess
import time
...
OUT = BASE / "pretrial_interface_restore_retest_once"
...
assert input("Keep drive POWER OFF. Type POWER_OFF to restore can0 for the retest: " ).strip() == "POWER_OFF"
pre = snapshot()
...
try:
    ...
    raise
print("DONE: can0 ERROR-ACTIVE. Keep drive POWER OFF; no soak started.", flush=True)
