...
probe = next(t for t in matching(0x601, '406C600100000000') if t > target)
...
result = {
    ...
    'physical_acceptance': 'INCONCLUSIVE: no initial motion observed; RX discrepancy and timing unresolved' if stationary else 'OPEN: operator observation, RX-counter discrepancy and exact timing',
    'observed_watchdog_behavior': 'both wheels observed stationary; dynamic stopping not demonstrated' if stationary else 'stop during silence followed by physical restart' if confirmed_restart else 'operator confirmation pending',
...
result['counter_discrepancy_window'] = 'During executor operation; readiness/shutdown boundary changes excluded'
