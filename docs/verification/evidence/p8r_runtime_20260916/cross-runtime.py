"""Reproduce the P8-R cross build with the locked image and real target sysroot."""
import hashlib
import json
import os
...
snapshot=root/'out/p8r-cross-source'
attestation=root/'out/p8r-cross-source.json'
...
configure=['cmake','-S','.','-B','out/build/cross/p8r-runtime','-G','Ninja',
...
subprocess.run(base+['cmake','--build','out/build/cross/p8r-runtime','--parallel','2'],check=True)
subprocess.run(['scripts/sysroot/validate_sysroot.sh',str(sysroot),str(lock)],check=True)
metadata={'source':json.loads(attestation.read_text()),'image_id':image,'configure':configure,
          ...
          'artifacts':{}}
for name in ('robot_control_canopen_runtime_vcan_tests','robot_control_canopen_runtime_policy_tests'):
 p=root/'out/build/cross/p8r-runtime'/name
 metadata['artifacts'][name]=hashlib.sha256(p.read_bytes()).hexdigest()
(root/'out/p8r-cross-metadata.json').write_text(json.dumps(metadata,indent=2)+chr(10))
