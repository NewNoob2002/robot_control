...
SERIAL = '207F346D5650'
BASE = '/tmp/robot-control-qualifications/cable-repair-acc9f8828f65'
SSH = ['ssh', '-o', 'BatchMode=yes', '-o', 'ConnectTimeout=5', 'robot-dev']
authorization = json.loads((OUT / 'authorization.json').read_text())
assert authorization['authorized'] is True and authorization['stimulus'] == 'cable_loss'
assert authorization['elf_sha256'] == 'acc9f8828f657d968cb2c56a586b1f9e1f2c50250f6c1385d3e6b7439abed46f'
...
def validate_frame(frame):
    ...
    global frames, downloads, uploads, nmts, nonzero, terminal, recovery, nmt_stop, startup_recovery, cleanup_disable
    frames += 1
...
try:
    ...
    while True:
        ...
        if label == 'jcan_out':
            ...
            if connected and receive_enabled and remote is None:
                remote = subprocess.Popen([*SSH, 'python3', '-u', BASE + '/remote_cable_loss_once.py'], stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
            ...
            if message == 'CAPTURE_READY':
                ...
                started = True
            elif message.startswith('APP ') or message in ('MANUAL_DISCONNECT_ARMED', 'OUTCOME INHIBITED_POWER_OFF_REQUIRED', 'OUTCOME PROTOCOL_RECOVERY_VERIFIED'):
                pass
            elif not message.startswith('TRIAL_FINISHED rc=0'):
                raise RuntimeError(message)
        elif label == 'jcan_out_eof':
            raise RuntimeError('JCAN disconnected')
    assert started and nonzero == 1 and terminal == 1 and nmt_stop == 0 and cleanup_disable in (0, 1)
    ...
    if remote is not None:
        transfer = subprocess.run(['scp', '-q', '-o', 'BatchMode=yes', '-o', 'ConnectTimeout=5', '-r', 'robot-dev:' + BASE + '/cable_loss_once', str(OUT / 'target')], capture_output=True, timeout=20)
