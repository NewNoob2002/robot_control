...
REMOTE = '/home/cat/.cache/robot-control/staging/p103-right-held-9ac673e9-a1-20260917'
...
def main():
    ...
    assert confirmation['statement'] == '右轮A1已上电就绪'
        ...
        with (BASE / 'motion-execution.log').open('wb') as log:
            ...
            while application.poll() is None:
                ...
                if not result.get('motion_prompted') and 'MOTION_READY:' in (BASE/'motion-execution.log').read_text():
                    ...
                    print('MOTION_READY: 右轮单轮；油门向前、转向向左同步推动，保持位置直到STOP_VERIFIED再回中；最多5rpm/3秒。', flush=True)
