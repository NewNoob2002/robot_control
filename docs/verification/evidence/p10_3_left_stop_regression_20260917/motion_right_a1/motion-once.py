"""One prepared right-wheel ControlLoop qualification on the explicitly identified RK3588."""
import hashlib
import json
import signal
import subprocess
...
ARGS = ['--interface', 'can0', '--device', '/dev/serial/by-id/usb-1a86_USB_Single_Serial_586D017868-if00', '--duration-ms', '60000', '--single-right']
            ...
            with (out / 'application.log').open('wb') as log:
                ...
                while application.poll() is None:
                    ...
                    if not result.get('operator_prompted') and 'event=control_start' in (out / 'application.log').read_text():
                        ...
                        print('CONTROL_READY: keep sticks neutral; press CH6 once; RIGHT trial window60s; wait for MOTION_READY before stick input', flush=True)
                    if not result.get('motion_prompted') and 'event=motion_ready' in (out / 'application.log').read_text():
                        result['motion_prompted'] = True
                        print('MOTION_READY: RIGHT only; forward throttle plus left steering; HOLD until STOP_VERIFIED; max5rpm/3s', flush=True)
                    if not result.get('stop_prompted') and 'event=motion_stop verified=' in (out / 'application.log').read_text():
                        result['stop_prompted'] = True
                        if 'event=motion_stop verified=1' in (out / 'application.log').read_text():
                            print('STOP_VERIFIED: zero feedback observed; now return sticks to neutral, keep wheels untouched', flush=True)
                        else:
                            print('STOP_NOT_VERIFIED: use emergency stop/power OFF immediately', flush=True)
                    time.sleep(0.02)
