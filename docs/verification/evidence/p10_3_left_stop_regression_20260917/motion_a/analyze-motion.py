...
def main():
    ...
    mode = auth['arguments'][-1]
    assert mode in ('--single-left', '--single-right')
    right = mode == '--single-right'
    nonzero = []
    zero_after = []
    velocities = []
    records = []
    ...
    for stamp, ident, payload in records:
        ...
        if ident == 0:
            ...
            assert other == 0 and 0 <= selected <= 75, 'feedback outside selected positive wheel envelope'
    ...
    log = (BASE/'motion-target/application.log').read_text()
    summary = re.search(r'event=summary cycles=(\d+) enabled_samples=(\d+).*primary_ok=1 restore_ok=1', log)
    ...
    assert all(speed == 0 for stamp,speed in velocities if stamp < nonzero[0])
    assert sum(speed > 0 for stamp,speed in velocities if nonzero[0] <= stamp <= zero_after[0]) >= 2
    stopped = [(stamp,speed) for stamp,speed in velocities if stamp >= zero_after[0]]
    last_moving = max((stamp for stamp,speed in stopped if speed), default=zero_after[0])
    stable = [stamp for stamp,speed in stopped if stamp > last_moving and speed == 0]
    ...
    (BASE/'motion-analysis.json').write_text(json.dumps(result,indent=2)+chr(10))
