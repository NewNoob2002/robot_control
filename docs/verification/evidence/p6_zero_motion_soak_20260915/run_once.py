#!/usr/bin/env python3
"""Execute the exact authorized three-hour zero-motion soak once."""
import hashlib
import json
import os
import sys
...
assert not OUTPUT.exists(), "one-shot soak already consumed"
marker = BASE / "run_once.marker"
marker.write_text(json.dumps({"wall_time": time.time(), "pid": os.getpid(),
                              "authorization_sha256": hashlib.sha256((BASE / "authorization.json").read_bytes()).hexdigest()},
                             indent=2) + "\n")
