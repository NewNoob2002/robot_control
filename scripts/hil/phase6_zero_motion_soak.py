...
def start_capture(interface, stdout, stderr):
    """Return an owned, receive-only capture isolated from terminal hangups."""
    return subprocess.Popen(["stdbuf", "-oL", "candump", "-D", "-ta", "-e",
                             f"{interface},0:0,#FFFFFFFF"], stdin=subprocess.DEVNULL,
                            stdout=stdout, stderr=stderr, start_new_session=True)


def assert_capture_running(capture):
    """Reject any premature capture exit, preserving its actual return code."""
    code = capture.poll()
...
def run_soak(args):
    ...
    try:
        with (output / "rk3588_can.log").open("wb") as can_log, (output / "candump.stderr").open("wb") as can_error, \
            ...
            capture = start_capture(args.interface, can_log, can_error)
            time.sleep(0.25)
            assert_capture_running(capture)
            assert not (output / "candump.stderr").read_text()
            cycle = 0
        ...
        result = {"status": "PASS" if complete else "INTERRUPTED" if STOP_REQUESTED and failure is None else "FAIL",
                  ...
                  "capture_returncode": None if capture is None else capture.returncode,
